...
interface SignInColumnManagementDialogProps {
  open: boolean;
  onClose: () => void;
}
...
export const SignInColumnManagementDialog: React.FC<SignInColumnManagementDialogProps> = ({
  open,
  onClose
}) => {
  const { state: settingsState, addSignInCustomColumn, removeSignInCustomColumn, toggleSignInColumnVisibility } = useSettings();
  ...
  const customColumns = settingsState.settings.signInDisplay.columnSettings.filter(col => col.isCustom);
  ...
  const validateNewColumn = (): string | null => {
    ...
    if (settingsState.settings.signInDisplay.columnSettings.some(col => col.field === newColumn.field)) {
  ...
  const handleAddColumn = async () => {
    ...
    try {
      ...
      await addSignInCustomColumn(columnConfig);
      ...
      setError(err instanceof Error ? err.message : 'Failed to add sign-in column');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteColumn = async (columnId: string) => {
    if (!confirm('Are you sure you want to delete this sign-in column? This action cannot be undone.')) {
      return;
    }
    ...
    try {
      await removeSignInCustomColumn(columnId);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to delete sign-in column');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Typography variant="h6">Manage Sign-In Custom Columns</Typography>
        <Typography variant="body2" color="text.secondary">
          Add, edit, or remove custom fields for sign-in records
        </Typography>
      </DialogTitle>
        ...
        <Box sx={{ py: 2 }}>
          ...
          <Paper sx={{ p: 2, mb: 3 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              <Typography variant="h6">Add New Sign-In Column</Typography>
              <Button
                startIcon={<AddIcon />}
          ...
          <Typography variant="h6" gutterBottom>
            Sign-In Custom Columns ({customColumns.length})
          </Typography>

          {customColumns.length === 0 ? (
            <Paper sx={{ p: 3, textAlign: 'center' }}>
              <Typography color="text.secondary" gutterBottom>
                No custom sign-in columns yet
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Add your first custom sign-in column to get started
              </Typography>
            </Paper>
          ) : (
            <List>
