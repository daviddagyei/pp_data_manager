...
/**
 * Service for managing Sign-In Google Sheets column structure and metadata
 */
export class SignInSheetColumnService {
  private readonly baseUrl = 'https://sheets.googleapis.com/v4';
  private readonly spreadsheetId: string;
  private readonly apiKey: string;
  private readonly sheetName = 'sign-ins';
  ...
  async getSheetHeaders(accessToken: string): Promise<string[]> {
    try {
      ...
      console.error('Error fetching sign-in sheet headers:', error);
      throw new Error('Failed to fetch sign-in sheet headers');
    }
  }
  ...
  async getSheetMetadata(accessToken: string): Promise<{
    ...
    try {
      ...
      console.error('Error fetching sign-in sheet metadata:', error);
      throw new Error('Failed to fetch sign-in sheet metadata');
    }
  }

  /**
   * Add a new column to the Sign-In Google Sheet
   */
  async addColumn(
    accessToken: string, 
    columnName: string, 
  ...
  /**
   * Rename a column in the Sign-In Google Sheet
   */
  async renameColumn(
    accessToken: string,
    ...
    try {
      ...
      if (columnIndex === -1) {
        throw new Error(`Column '${oldColumnName}' not found in sign-in sheet`);
      }
      ...
      console.log(`Successfully renamed sign-in column from '${oldColumnName}' to '${newColumnName}'`);
    } catch (error) {
      console.error('Error renaming sign-in column:', error);
      throw new Error(`Failed to rename sign-in column: ${error}`);
    }
  }

  /**
   * Remove a column from the Sign-In Google Sheet
   */
  async removeColumn(accessToken: string, columnName: string): Promise<void> {
    try {
      ...
      if (columnIndex === -1) {
        throw new Error(`Column '${columnName}' not found in sign-in sheet`);
      }
      ...
      console.log(`Successfully removed column '${columnName}' from sign-in sheet`);
    } catch (error) {
      console.error('Error removing sign-in column:', error);
      throw new Error(`Failed to remove sign-in column: ${error}`);
    }
  }

  /**
   * Reorder columns in the Sign-In Google Sheet
   */
  async reorderColumns(
    ...
    try {
      ...
      console.error('Error reordering sign-in columns:', error);
      throw new Error(`Failed to reorder sign-in columns: ${error}`);
    }
  }

  /**
   * Sync column settings with Sign-In Google Sheets structure
   */
  async syncColumnSettings(
    ...
    try {
      ...
      console.error('Error syncing sign-in column settings:', error);
      throw new Error(`Failed to sync sign-in column settings: ${error}`);
    }
  }

  /**
   * Check which custom columns from settings are missing in Sign-In Google Sheets
   */
  async detectMissingCustomColumns(
    ...
    try {
      ...
      console.error('Error detecting missing sign-in custom columns:', error);
      throw new Error(`Failed to detect missing sign-in custom columns: ${error}`);
    }
  }
  ...
  private async columnHasData(accessToken: string, columnName: string): Promise<boolean> {
    try {
      ...
      console.error('Error checking sign-in column data:', error);
  ...
  private isSystemColumn(columnName: string): boolean {
    const systemColumns = [
      'firstname', 'lastname', 'school', 'phone', 'gradyear', 'email', 'date', 'event'
    ];
...
export default SignInSheetColumnService;
